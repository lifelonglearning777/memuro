...
const questions = [
  ...
  { level:"beginner", question:"じゃがいもの別のよび方は？", image:"potato.jpg", choices:["ばれいしょ","てん菜","小麦"], answer:0, explanation:"ばれいしょは、じゃがいものことです。", source:"めむろ学 p.26–31" },
  ...
  { level:"beginner", question:"砂糖の材料になる作物は？", image:"sugar-beet.jpg", choices:["てん菜（ビート）","たまねぎ","大豆"], answer:0, explanation:"てん菜は砂糖の大切な原料です。", source:"めむろ学 p.26–35" },
  ...
  { level:"intermediate", question:"ビートから作られるものは？", image:"sugar-beet.jpg", choices:["砂糖","紙だけ","ガラス"], answer:0, explanation:"製糖工場では、てん菜から砂糖を作ります。", source:"めむろ学 p.40–49" },
...
function startQuiz(level, queue = buildQuestionQueue(level), reviewMode = false) {
  state = { ...state, level, queue, index: 0, score: 0, wrong: [], reviewMode, locked: false, currentChoices: [] };
...
function renderQuestion() {
  ...
  setKidHtml($("questionText"), item.question);
  ...
  $("choiceList").replaceChildren(...state.currentChoices.map((choice, index) => {
    ...
    button.innerHTML = `<span class="choice-letter">${String.fromCharCode(65 + index)}</span><span>${kidHtml(choice.text)}</span>`;
